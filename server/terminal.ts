...
export class TerminalManager {
  ...
  async run(socket: Socket, payload: unknown) {
    ...
    try {
      ...
      const { command, args, displayCommand } = await this.buildCommand({ language, code, workDir });
      this.emit(roomId, `\r\n$ ${displayCommand}\r\n`);
      ...
      proc.on('close', (exitCode, signal) => {
        ...
        if (killedForLimit) {
          this.io.to(roomId).emit('terminal:exit', { exitCode: -1, signal: undefined });
          return;
        }
      ...
      proc.on('error', (err) => {
        clearTimeout(timeout);
        session.proc = null;
        socket.emit('terminal:error', { error: err instanceof Error ? err.message : String(err) });
        this.maybeScheduleCleanup(session);
      });
    } catch (err: unknown) {
  ...
  private normalizeLanguage(value: unknown): 'python' | 'javascript' | 'typescript' | null {
  ...
  private async prepareWorkDir(session: RoomTerminal, roomId: string) {
    ...
    const workDir = await fs.mkdtemp(path.join(os.tmpdir(), `codelink-${roomId}-${random}-`));
  ...
  private async buildCommand(opts: {
    ...
    if (language === 'python') {
      ...
      return { command: 'python3', args: ['-u', 'main.py'], displayCommand: 'python3 -u main.py' };
    }

    if (language === 'javascript') {
      const file = path.join(workDir, 'main.js');
      await fs.writeFile(file, code, 'utf8');
      return { command: 'node', args: ['main.js'], displayCommand: 'node main.js' };
    }
    ...
    return { command: 'node', args: ['main.js'], displayCommand: 'node main.js  # (from TS transpile)' };
  }
  ...
  private dispose(roomId: string) {
    ...
    if (session.workDir) {
      ...
      fs.rm(session.workDir, { recursive: true, force: true }).catch(() => undefined);
